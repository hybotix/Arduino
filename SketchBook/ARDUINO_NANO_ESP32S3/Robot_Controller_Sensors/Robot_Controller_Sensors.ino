...
String timestamp (bool show_full_date=SHOW_FULL_DATE, bool hours_mode=SHOW_24_HOURS, bool long_date=SHOW_LONG_DATE, bool show_seconds=SHOW_SECONDS) {
  DateTime date_time_info;
  time_t current_time;
  struct tm* timeinfo;
  char time_date_str[80];

  //  Read the current time from the onboard RTC
  current_time = time(nullptr);
  timeinfo = localtime(&current_time);
  strftime(time_date_str, 80, "%m/%d/%Y at %H:%M:%S", timeinfo);
  Serial.print("Clock time = '");
  Serial.print(time_date_str);
  Serial.println("'");
...
*/
String switch_format_str (uint8_t nr_of_switches=NUMBER_OF_SWITCHES) {
  uint8_t index;
  String html = "", state;
...
//  delay(5000);
  ...
  if (error) {
    ...
    if (error) {
      ...
      if (error) {
        Serial.print("SCD-40 Data is not ready");
        delay(100);
      } else {
        while ((error) and (attempt_count < 5)) {
          ...
          if (error) {
            Serial.print("Unable to start measurment: Code: ");
        ...
        if (error) {
          Serial.print("Unable to read from the SCD-40, Code = ");
          Serial.println(error);
       }
      }
    }
...
*/
String imu_format_xyz_str (Three_Axis readings) {
...
Environment_Data get_scd40 (Environment_Data curr_data, SensirionI2CScd4x *scd) {
  Environment_Data sensors;
  String error_message;
  uint16_t CO2, error = 1, attempt_count = 0;
  float celsius, fahrenheit, humidity;

  sensors = check_data(curr_data);

  while (((error) and (attempt_count < 5)) or (CO2 == 0)) {
    attempt_count++;

    // Read Measurement
    Serial.print("Attempt #");
    Serial.print(attempt_count);
    Serial.println(" to read the SCD-40");
    ...
    if (error) {
      ...
      if (error) {
        ...
        error_message = "Unable to read the SCD-40, code " + String(error);
...
void setup (void) {
  ...
  time_t current_time;
  struct tm* timeinfo;
  char buffer[80], time_date_str[80];
...
*/
  ...
  //  Initialize the LIS3MDL Magnetometer - contained on the LSM6DSOX 9DOF IMU
  ...
  // Set the onboard real-time clock to the current time
  unix_time = date_time_info.unixtime();
  set_time(unix_time);
...
void loop (void) {
  ...
  String date_time = "", temp_str = "", led_html = "";
  ...
  while (true) {
    ...
    if (USING_SCD40_CO2) {
      ...
      if (sensors.scd40.valid) {
        Serial.print("SCD-40: Temperature = ");
        Serial.print(sensors.scd40.fahrenheit);
        Serial.print(" F (");
        Serial.print(sensors.scd40.celsius);
        Serial.print(" C), Humidity: ");
        Serial.print(sensors.scd40.humidity);
        Serial.println("% rH");
    ...
    if (USING_ENS160_MOX) {
      ...
      if (sensors.ens160.valid) {
        ...
        Serial.print(" ppb, Equivalent CO2: ");
        Serial.print(sensors.ens160.eCO2);
        Serial.println(" ppm");
     } else {
    ...
    if (USING_LSM6DSOX_LIS3MDL_IMU) {
      ...
      //  Accelerometer
      temp_str = imu_format_xyz_str(sensors.lsm6dsox.accelerometer);
      Serial.print("Accelerometer = ");
      Serial.println(temp_str);
      //  Gyroscope
      temp_str = imu_format_xyz_str(sensors.lsm6dsox.gyroscope);
      Serial.print("Gyroscope = ");
      Serial.println(temp_str);

      //  Magnetometer (LIS3MDL)
      Serial.print("Magnetometer = ");
      temp_str = imu_format_xyz_str(sensors.lsm6dsox.magnetometer);
      Serial.println(temp_str);
    }
