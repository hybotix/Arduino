...
#if defined(ARDUINO_GIGA) || defined(ARDUINO_PORTENTA_H7)
    /*
        Enable the Giga R1 host USB port and try connecting
          to the robot controller board.
    */
  Serial.println("Enabling the Arduino Giga R1 USB host port");
  pinMode(PA_15, OUTPUT);
  digitalWrite(PA_15, HIGH);
  
  // Attempt to connect to the robot controller board
  if (robot.connect()) {
    //  Set baud rate and format
    robot.baud(115200);
    robot.format(8, USBHostSerialPort::None, 1);
    Serial.println("Connected to USB device");
  } else {
    Serial.println("Failed to connect to a USB device");
  }
#endif
